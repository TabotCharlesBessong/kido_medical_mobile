...
const PrescriptionDetailsScreen: React.FC = () => {
  ...
  if (!prescription) {
    return <CustomText type='h1'>{t('prescriptionDetails.notFound')}</CustomText>
  }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <CustomText type='h4'>{`${t('prescriptionDetails.instructions')} ${
        prescription.instructions || t('prescriptionDetails.none')
      }`}</CustomText>
      <CustomText type='h4'>{`${t('prescriptionDetails.investigation')} ${
        prescription.investigation || t('prescriptionDetails.none')
      }`}</CustomText>
      <CustomText type='h2'>{t('prescriptionDetails.medications')}</CustomText>
      {prescription?.medications?.map(med => (
        <View key={med.id} style={styles.medicationCard}>
          <CustomText type='h4'>{`${t(
            'prescriptionDetails.medicationName',
          )} ${med.name}`}</CustomText>
          <CustomText type='h4'>{`${t('prescriptionDetails.dosage')} ${med.dosage}`}</CustomText>
          <CustomText type='h4'>{`${t(
            'prescriptionDetails.frequency',
          )} ${med.frequency}`}</CustomText>
          <CustomText type='h4'>{`${t('prescriptionDetails.durationPrefix')} ${med.duration} ${t(
