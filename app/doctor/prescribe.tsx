...
const CreatePrescriptionScreen: React.FC = () => {
  ...
  const initialValues: PrescriptionValues = {
    instructions: '',
    investigation: '',
    medications: [{ name: '', dosage: '', frequency: Frequency.ONCE_A_DAY, duration: '' }],
  }

  const prescriptionSchema = yup.object().shape({
    instructions: yup.string().required(t('createPrescription.validation.instructionsRequired')),
    investigation: yup.string().required(t('createPrescription.validation.investigationRequired')),
    medications: yup
      .array()
      .of(
        yup.object().shape({
          name: yup.string().required(t('createPrescription.validation.medicationNameRequired')),
          dosage: yup.string().required(t('createPrescription.validation.dosageRequired')),
          frequency: yup
            .string()
            .oneOf(['ONCE_A_DAY', 'TWICE_A_DAY', 'THRICE_A_DAY'])
            .required(t('createPrescription.validation.frequencyRequired')),
          duration: yup.string().required(t('createPrescription.validation.durationRequired')),
        }),
      )
                  ...
                  {values.medications.map((_, index) => (
                    <View key={index} style={styles.medicationContainer}>
                      <AuthInputField
                        ...
                        placeholder={t('createPrescription.placeholders.medicationName')}
                      />
                      <AuthInputField
                        name={`medications[${index}].dosage`}
                        label={t('createPrescription.fields.dosage')}
                        placeholder={t('createPrescription.placeholders.dosage')}
                      />
                      <AuthInputField
                        name={`medications[${index}].frequency`}
                        label={t('createPrescription.fields.frequency')}
                        placeholder={t('createPrescription.placeholders.frequency')}
                      />
                      <AuthInputField
                        name={`medications[${index}].duration`}
                        label={t('createPrescription.fields.duration')}
                        placeholder={t('createPrescription.placeholders.duration')}
                      />
                      <AppButton
