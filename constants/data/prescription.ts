...
const frequencies = ['Once a day', 'Twice a day', 'Three times a day', 'Every 6 hours']
...
export const generatePrescription = (numPrescriptions: number): IPrescription[] => {
  ...
  for (let i = 0; i < numPrescriptions; i++) {
    ...
    mockData.push({
      ...
      instructions: Math.random() > 0.5 ? getRandomElement(instructions) : undefined,
      investigation: Math.random() > 0.5 ? getRandomElement(investigations) : undefined,
      medications: meds,
      createdAt: new Date(),
      updatedAt: new Date(),
